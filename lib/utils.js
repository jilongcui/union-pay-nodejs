...
utils.buildParams = function(obj, context) {
    var params = {
        ...
        backUrl: context.backUrl,           // 后台通知地址
 ...
 * @return {Object}
 */
utils.buildBizParams = function(obj, context) {
    var params = {
        version: '5.0.0',       // 版本号，固定
        encoding: 'UTF-8',
        bizType: obj.bizType,               // 产品类型
        ...
        channelType: '08',                  // 渠道类型
        merId: context.merId,               // 商户ID
        bizScene:context.bizScene,        // 业务种类
        payeeAcctNo:context.payeeAcctNo,  // 收款方账号
        payeeAcctNm:context.payeeAcctNm,  // 收款方名称
        payeeBankName:context.payeeBankName, // 收款银行名称
        frontUrl: context.frontUrl,         // 前台通知地址
        backUrl: context.backUrl,           // 后台通知地址
        certId: context.certId,             // 证书ID
        orderId: obj.orderId,               // 商户订单号
...
utils.buildAutoFormHtml = function(params, reqUrl) {
    const encodeType = params.encoding;
    let html = `<html> <head> <meta http-equiv="Content-Type" content="text/html; charset=${encodeType}" /> </head> <body onload="javascript:document.pay_form.submit();"> <form id="pay_form" name="pay_form" action="${reqUrl}" method="post">`;
